...
?>
    <tr>
        ...
        <td style="text-align:right; color:#2c3e50;">
            <?php 
            $stmt_monto = $conn->prepare("SELECT COALESCE(SUM(total_usd_bo), 0) as monto_total FROM items WHERE id_proyecto = ?");
            $stmt_monto->execute([$proyecto['id_proyecto']]);
            $monto = $stmt_monto->fetch(PDO::FETCH_ASSOC);
            ?>
            Bs <?= number_format($monto['monto_total'] ?? 0, 2, ',', '.') ?>
        </td>
