...
if (!$anio) {
    header("HTTP/1.1 404 Not Found");
    echo json_encode(["error" => 1, "message" => "No se encontró el año para el proyecto"]);
    exit;
}

$dir = "/mnt/files/adjuntos_presupuestos/{$anio['anio']}";

$filePath = "{$dir}/hoja_{$numero_proyecto}.xlsx";
...
if (isset($data['url'])) {
    ...
    if (file_put_contents($filePath, $fileContent) === false) {
        header("HTTP/1.1 500 Internal Server Error");
        echo json_encode(["error" => 1, "message" => "No se pudo guardar el archivo en {$filePath}"]);
        exit;
    }
}
