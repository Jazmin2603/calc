            ...
            <div style="background: white; border-radius: 12px; padding: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
                <table class="costos-table">
                    ...
                    <tbody>
                        ...
                        $monto_acumulado = 0;
                        $stmt = $conn->prepare("
                            SELECT margen_desde, margen_hasta, porcentaje
                            FROM comision_margen_producto
                            WHERE gestion_id = ? AND activo = 1
                            ORDER BY margen_desde ASC
                        ");
                        $stmt->execute([$gestion_seleccionada]);
                        $rangos_margen = $stmt->fetchAll(PDO::FETCH_ASSOC);

                        $stmt = $conn->prepare("
                            SELECT porcentaje_meta_desde, porcentaje_extra
                            FROM comision_meta_extra
                            WHERE gestion_id = ? AND activo = 1
                            ORDER BY porcentaje_meta_desde DESC
                        ");
                        $stmt->execute([$gestion_seleccionada]);
                        $rangos_meta = $stmt->fetchAll(PDO::FETCH_ASSOC);
                        ...
                        foreach ($proyectos as $proyecto): 
                            ...
                            foreach ($rangos_margen as $rango) {
                                if (
                                    $margen_prod >= $rango['margen_desde'] &&
                                    ($rango['margen_hasta'] === null || $margen_prod < $rango['margen_hasta'])
                                ) {
                                    $por_prod = (float)$rango['porcentaje'];
                                    break;
                                }
                            }
                            ...
                            foreach ($rangos_meta as $rango) {
                                if ($porcentaje_meta >= $rango['porcentaje_meta_desde']) {
                                    $por_prod_extra = (float)$rango['porcentaje_extra'];
                                    break;
                                }
                            }
                    ...
                    <tfoot>
                        <tr style="background: #f8f9fa; font-weight: bold;">
                            <td colspan="4" style="text-align: right; padding: 20px;">TOTALES:</td>
